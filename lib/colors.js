...
function normalizeInlineColorsInHtml(html) {
  ...
  out = out.replace(/\bstyle\s*=\s*(?:"([^"]*)"|'([^']*)')/gi, (full, dq, sq) => {
    ...
    // Security-Nachbesserung (2. Re-Review): ' ' statt '' - ein ersatzlos
    // entferntes style="..." kann Zeichen davor/danach (z.B. Attributnamen
    // eines benachbarten Tags) wieder zusammenwachsen lassen.
    if (!sanitized) return ' ';

...
// IPv6-Bereiche: ::/128, ::1/128, fc00::/7, fe80::/10, ff00::/8, plus
// Übersetzungspräfixe, über die eine private/loopback-IPv4-Adresse in
// Netzen mit NAT64/DNS64 bzw. 6to4-Relay erreichbar sein kann (Security-
// Review-Fund): IPv4-kompatibel (::/96), IPv4-translated (::ffff:0:0:0/96,
// RFC 2765 — NICHT zu verwechseln mit IPv4-mapped ::ffff:0:0/96, das
// absichtlich NICHT hier steht, sondern per unmapIPv4MappedIPv6() gegen die
// IPv4-Liste geprüft wird, damit legitime ::ffff:<öffentliche IPv4>-Ziele
// erreichbar bleiben), NAT64 (64:ff9b::/96), 6to4 (2002::/16).
const BLOCKED_V6_SUBNETS = [
  ...
  ['ff00::', 8],
  ['::', 96],
  // Ausgeschriebene Form von ::ffff:0:0:0/96 (IPv4-translated, RFC 2765) —
  // node:net.BlockList behandelt die "::"-Kompression an dieser Stelle beim
  // Subnetz-Eintrag nicht wie erwartet (verifiziert: Adressen im Bereich
  // matchten sonst nicht).
...
function unmapIPv4MappedIPv6(address) {
  // Kanonisiert zuerst über den WHATWG-URL-Parser (RFC-5952-konforme
  // Kompression), bevor das Muster geprüft wird - Security-Review-Fund:
  // reines Regex-Matching auf zwei feste Textformen ("::ffff:..." bzw. voll
  // ausgeschrieben) erkennt beliebige weitere gültige Zwischenformen (z.B.
  // "0::ffff:127.0.0.1", "0:0::ffff:127.0.0.1") nicht. Nach der
  // Kanonisierung liefert der Parser für alle gültigen IPv4-mapped-Formen
  // immer dieselbe komprimierte "::ffff:xxxx:xxxx"-Darstellung.
  let a = String(address || '').toLowerCase();
  if (!net.isIPv6(a)) return null;
  try {
    a = new URL(`http://[${a}]/`).hostname.replace(/^\[|\]$/g, '');
  } catch { /* Kanonisierung fehlgeschlagen -> unten mit Original weiterprüfen */ }
